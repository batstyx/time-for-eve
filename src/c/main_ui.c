...
static void configure_background(Layer *parent_layer, GRect bounds) {
  s_background_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_BACKGROUND);
  
  s_background_layer = bitmap_layer_create(GRect((bounds.size.w - EVE_E_WIDTH) / 2, (bounds.size.h - EVE_E_HEIGHT) / 2, EVE_E_WIDTH, EVE_E_HEIGHT));
...
static void init_text_layer(Layer *parent_layer, TextLayer **text_layer, int16_t left, int16_t top, int16_t width, int16_t height, char *font_key, GTextAlignment alignment) {
...
void main_window_load(Window *window) {
  ...
  Layer *parent_layer = bitmap_layer_get_layer(s_background_layer);
  
  init_text_layer(parent_layer, &data->eve_time_layer, 
  ...
  init_text_layer(parent_layer, &data->user_count_layer,
                  USER_COUNT_LEFT, USER_COUNT_TOP, USER_COUNT_WIDTH, USER_COUNT_HEIGHT, 
                  USER_COUNT_FONT, USER_COUNT_ALIGN);
  init_text_layer(parent_layer, &data->service_status_layer, 
  ...
  init_text_layer(parent_layer, &data->market_item_desc_layer, 
  ...
  init_text_layer(parent_layer, &data->market_item_value1_layer, 
  ...
  init_text_layer(parent_layer, &data->market_item_value2_layer, 
  ...
  init_text_layer(parent_layer, &data->market_item_value3_layer, 
